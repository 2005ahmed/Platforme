  ...
  return (
    <TopLayout title="Recruiter Panel">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              {activeTab === "applications"
                ...
                : "Suivez les emails envoyés aux candidats"}
            </p>
          </div>
        </div>
                ...
                <tbody>
                  {filtered.map((app) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4">
                        <span
                          className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border ${statusStyle(
                            app.status
                          )}`}
                        >
                      ...
                      <tr key={h.id} className="border-b border-slate-100 hover:bg-slate-50">
                        ...
                        <td className="py-4 px-4">
                          <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${
                            ...
                            {h.status === "opened" && <Eye size={12} />}
                            {h.status === "sent" && <Send size={12} />}
                        ...
                        <td className="py-4 px-4 text-slate-500 text-sm">
                          {h.opened_at ? (
                            <span className="text-green-600 font-medium">
                              {new Date(h.opened_at).toLocaleString("fr-FR", {
                                day: "2-digit",
                                month: "short",
                                hour: "2-digit",
                                minute: "2-digit"
      ...
      {showCVModal && selectedCV && (
        ...
        >
          <div 
            className="bg-white w-full max-w-5xl h-[95vh] rounded-3xl p-6 shadow-2xl flex flex-col"
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {selectedCandidate?.full_name} — {selectedCV}
                </p>
              </div>
              <div className="flex gap-2">
                <a
                  href={`${BACKEND_URL}/uploads/${selectedCV}`}
                  download={selectedCV}
                  className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-500 text-sm"
                  onClick={(e) => e.stopPropagation()}
                >
                  <Download size={16} />
                  Télécharger
            ...
            <div className="flex-1 overflow-hidden rounded-xl border border-slate-200 bg-white">
              {selectedCV.endsWith(".pdf") ? (
                <iframe
                  src={`${BACKEND_URL}/uploads/${selectedCV}`}
                ...
                <div className="flex flex-col items-center justify-center h-full py-10">
                  ...
                  <p className="text-sm text-slate-400 mb-6">
                    Format DOCX — Prévisualisation non disponible
                  </p>
                  <a 
                    href={`${BACKEND_URL}/uploads/${selectedCV}`}
                    download={selectedCV}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 shadow-2xl">
            ...
            <button
              ...
              className="w-full py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-semibold"
            >
              Envoyer Notification
            </button>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 shadow-2xl">
            ...
            <div className="mb-4">
              ...
              <p className="font-medium text-slate-900">
                {selectedApp.candidate?.full_name} ({selectedApp.candidate?.email})
              </p>
            </div>
            ...
            <button
              onClick={sendEmail}
              className="w-full py-3 rounded-2xl bg-green-600 hover:bg-green-500 text-white font-semibold"
            >
              📧 Envoyer Email de Relance
            </button>
          </div>
