...
export default function ApplicationsPage() {
  ...
  // ApplicationsPage.jsx - Zid console.log bach t-debug
...
const load = async () => {
    try {
      ...
      setUserCV(profileRes.data.cv_filename || null);
    } catch (err) {
      console.log(err);
    }
  };
    ...
    <Layout title="Candidatures">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          {/* SE7A7: Ghir user y9der y-zid candidature */}
          {user?.role === "user" && (
            <button
              onClick={() => {
                setEditId(null);
  ...
  <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
    <div className="bg-white w-full max-w-xl rounded-3xl p-6 shadow-2xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setOpenModal(false)} className="text-slate-500 hover:text-black text-xl">
          ✕
        </button>
      </div>
      ...
      <div className="space-y-4">
        ...
        {user?.role === "recruiter" ? (
          ...
          <div className="w-full px-4 py-3 rounded-2xl border border-slate-200 bg-slate-100 text-slate-500">
        ...
        >
          {editId ? "Modifier" : "Ajouter la candidature"}
        </button>
      </div>
    </div>
  </div>
